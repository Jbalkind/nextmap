...
# Add current directory to path for nextmap import (only needed when nextmap is not
# installed as a package; harmless otherwise).
sys.path.insert(0, os.getcwd())


def default_schema_path():
    """Locate the schema shipped with the installed nextmap package.

    Falls back to ./nextmap/schema.sql for in-tree (non-installed) use.
    """
    try:
        import nextmap
        pkg_schema = os.path.join(os.path.dirname(nextmap.__file__), "schema.sql")
        if os.path.exists(pkg_schema):
            return pkg_schema
    except Exception:
        pass
    return "./nextmap/schema.sql"
...
def run_nextmap(input_file, output_file, schema_path, max_iter=10, strategy="basic"):
    ...
    try:
        import nextmap
        from nextmap.extracts.utils import db_to_normalized, normalized_to_json
        ...
        netlist = nextmap.NetlistDB(schema_path)
        ...
        for iteration in range(max_iter):
            ...
            try:
                if strategy == "retiming":
                    ...
                    matches = nextmap.rewrites.ematch_dff_forward_aby_cell(netlist, ['$adds', '$addu', '$muls', '$mulu'])
                    count = nextmap.rewrites.apply_dff_forward_aby_cell(netlist, matches)
                    ...
                    try:
                        wide_mul_matches = nextmap.rewrites.ematch_wide_muls(netlist)
                        wide_mul_count = nextmap.rewrites.apply_wide_muls_split(netlist, wide_mul_matches)
                        if wide_mul_count > 0:
                            print(f"Applied {wide_mul_count} wide multiplication splits")
                            netlist.rebuild()

                        wide_dff_matches = nextmap.rewrites.ematch_wide_dff(netlist)
                        wide_dff_count = nextmap.rewrites.apply_wide_dff_split(netlist, wide_dff_matches)
                        if wide_dff_count > 0:
                    ...
                    iteration_rewrites += nextmap.rewrites.apply_comm(netlist, comm_matches)

                    dff_forward_matches = nextmap.rewrites.ematch_dff_forward_aby_cell(netlist, ['$adds', '$muls'])
                    iteration_rewrites += nextmap.rewrites.apply_dff_forward_aby_cell(netlist, dff_forward_matches)

                    dff_backward_matches = nextmap.rewrites.ematch_dff_backward_aby_cell(netlist, ['$adds', '$muls'])
                    iteration_rewrites += nextmap.rewrites.apply_dff_backward_aby_cell(netlist, dff_backward_matches)

                elif strategy == "comprehensive":
                    # Apply multiple rewrite rules like in the notebook
                    # Commutativity
                    comm_matches = nextmap.rewrites.ematch_comm(netlist, ['$adds', '$addu', '$muls', '$mulu'])
                    iteration_rewrites += nextmap.rewrites.apply_comm(netlist, comm_matches)

                    # Associativity
                    assoc_right_matches = nextmap.rewrites.ematch_assoc_to_right(netlist, ['$adds', '$addu', '$muls', '$mulu'])
                    iteration_rewrites += nextmap.rewrites.apply_assoc_to_right(netlist, assoc_right_matches)

                    assoc_left_matches = nextmap.rewrites.ematch_assoc_to_left(netlist, ['$adds', '$addu', '$muls', '$mulu'])
                    iteration_rewrites += nextmap.rewrites.apply_assoc_to_left(netlist, assoc_left_matches)

                    # DFF retiming
                    dff_forward_matches = nextmap.rewrites.ematch_dff_forward_aby_cell(netlist, ['$adds', '$addu', '$muls', '$mulu'])
                    iteration_rewrites += nextmap.rewrites.apply_dff_forward_aby_cell(netlist, dff_forward_matches)

                    dff_backward_matches = nextmap.rewrites.ematch_dff_backward_aby_cell(netlist, ['$adds', '$addu', '$muls', '$mulu'])
                    iteration_rewrites += nextmap.rewrites.apply_dff_backward_aby_cell(netlist, dff_backward_matches)
        ...
        if strategy == "dsp":
            try:
                ...
                optimized_module = nextmap.extracts.ilp.extract_techmap_with_limit(
...
def main():
    ...
    parser.add_argument('--schema', default=None,
                        help='Schema file path (default: schema shipped with the nextmap package)')
    ...
    # Use the provided schema if it exists; otherwise fall back to the schema
    # shipped with the installed nextmap package. This lets the Yosys plugin
    # pass a placeholder path and still work once nextmap is installed.
    schema = args.schema if (args.schema and os.path.exists(args.schema)) else default_schema_path()
