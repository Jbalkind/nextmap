# nextmap Database Schema Management

## Overview

nextmap supports both SQLite and PostgreSQL backends with automatic schema conversion. **You only need to maintain the SQLite schema** - the PostgreSQL schema is automatically generated.
...
db = nextmap.NetlistDB('nextmap/schema.sql', 'mydb.db', backend='sqlite')

# PostgreSQL (auto-generates schema_postgres.sql if needed)
db = nextmap.NetlistDB('nextmap/schema.sql', backend='postgres')
```
...
```bash
python3 nextmap/schema_converter.py nextmap/schema_sqlite.sql
```
