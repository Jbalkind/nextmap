...
def analyze_lut_quality():
    ...
    for lut_count in lut_counts:
        ...
        with contextlib.redirect_stdout(f):
            nextmap.rewrites.techmap_luts(netlist, k=6, cnt=lut_count, rseed=42, greedy=True, verbose=True)
    ...
    for mode, greedy in [("Greedy", True), ("Random", False)]:
        ...
        start = time.time()
        with contextlib.redirect_stdout(f):
            nextmap.rewrites.techmap_luts(netlist, k=6, cnt=100, rseed=42, greedy=greedy, verbose=True)
        elapsed = (time.time() - start) * 1000
