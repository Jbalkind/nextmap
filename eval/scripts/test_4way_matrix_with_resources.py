#!/usr/bin/env python3
"""
4-way test matrix with resource counting: SQLite/PostgreSQL × Gurobi/CBC
...
def run_systolic_test(test_name: str, dsp_limit: int, database_backend: str, solver_backend: str) -> dict:
    ...
    try:
        ...
        # Phase 2: Apply rewrites (based on eval_systolic.ipynb)
        ...
        # Phase 3: Tech mapping (using eval_systolic.ipynb approach)
        techmap_start = time.time()
        nextmap.rewrites.create_tech_tables(netlist, dsp_rules)
        nextmap.rewrites.rewrite_tech(netlist, dsp_rules)  # Different from eval_systolic_v2.ipynb
        techmap_time = time.time() - techmap_start

        # Phase 4: ILP extraction with specified solver
        ...
        # Phase 5: Generate extracted JSON and run yosys synthesis for resource counting
...
def run_4way_test():
        ...
        for solver_backend in solver_backends:
            ...
            try:
                ...
                if result['success']:
                    ...
                    print(f"     Rewrites: {result['total_rewrites']}")
    ...
    if successful_configs:
        ...
        print(f"{'Config':<18} {'Time(s)':<8} {'Rewrites':<9} {'DSPs':<5} {'LUTs':<6} {'FFs':<6} {'BRAMs':<6}")
        print("-" * 70)
        for result in successful_configs:
            resources = result.get('resources', {}).get('extracted', {})
            print(f"{result['config']:<18} {result['total_time']:<8.2f} {result['total_rewrites']:<9} {result['dsps']:<5} {resources.get('luts', 0):<6} {resources.get('ffs', 0):<6} {resources.get('brams', 0):<6}")
        print()
    ...
    with open('4way_test_results_with_resources.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nDetailed results saved to: 4way_test_results_with_resources.json")
